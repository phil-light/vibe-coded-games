...
// Rain effect
const RAIN_SPEED = 6;       // pixels per frame; matches player fall speed
...
const LEVELS = [
  ...
  // Level 4: Precision platforming with gravity zones
  // ABCDEFGHIJKLMNOPQRS
  [
    '###################', // 1
    '#X...#............#', // 2
  ...
  // Level 5: Enemies meet gravity (The Vertical Pendulum)
  // ABCDEFGHIJKLMNOPQRS
  [
    '###################', // 1
    '#X......#....v....#', // 2
  ...
  // Level 6: The Lateral Pendulum (Horizontal Gravity Shifts)
  // ABCDEFGHIJKLMNOPQRS
  [
    '###################', // 1
    '#>.......E.......<#', // 2
  ...
  // Level 8: Puzzle-like gravity sequence
  // ABCDEFGHIJKLMNOPQRS
  [
    '###################', // 1
    '#......X..........#', // 2
  ...
  // Level 9: Expert mix - enemies and precision
  // ABCDEFGHIJKLMNOPQRS
  [
    '###################', // 1
    '#X...E.........#..#', // 2
...
function updateGame() {
  ...
  // Arrow blocks: Player AND enemies trigger gravity changes.
  for (const a of arrows) {
    if (a.contains(player.x, player.y) && gravityDir !== a.dir) {
      gravityDir = a.dir;
    }
    for (const e of enemies) {
      if (a.contains(e.x, e.y) && gravityDir !== a.dir) {
        gravityDir = a.dir;
      }
    }
  }
...
function initRain() {
  ...
  for (let i = 0; i < NUM_RAINDROPS; i++) {
    raindrops.push({ x: random(width), y: random(height) });
  }
}
...
function drawRain() {
  ...
  stroke(200, 215, 230, 90);
  strokeWeight(1);

  for (const drop of raindrops) {
    drop.x += g.x * RAIN_SPEED;
    drop.y += g.y * RAIN_SPEED;

    // Wrap to the opposite edge, randomising the perpendicular position
    if (drop.y > height) { drop.y = 0;      drop.x = random(width); }
    if (drop.y < 0)      { drop.y = height; drop.x = random(width); }
    if (drop.x > width)  { drop.x = 0;      drop.y = random(height); }
    if (drop.x < 0)      { drop.x = width;  drop.y = random(height); }

    line(drop.x - g.x * len, drop.y - g.y * len,
         drop.x + g.x * len, drop.y + g.y * len);
  }
}
