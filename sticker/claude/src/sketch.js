...
function draw() {
  ...
  if (gameState === 'title') {
    ...
    drawGame();
    drawBanner('You reached the portal!', 'Press R to play again',
               color(180, 120, 240));
  } else if (gameState === 'lost') {
    drawGame();
    drawBanner('Ouch! A star got you.', 'Press R to try again',
               color(220, 80, 80));
  }
}
...
function drawTitle() {
  ...
  textSize(54);
  text('STICKER', width / 2, height * 0.48);
  textSize(18);
  text('a gravity-flip platformer', width / 2, height * 0.56);
  ...
  if ((tick % 60) < 40) {
    fill(255, 240, 100);
    textSize(20);
    text('Press ENTER to start', width / 2, height * 0.90);
  }
}
...
function drawWalls() {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      fill(120, 80, 40);            // brown dirt
      rect(x, y, TILE, TILE);
      // Grass goes on the face opposite gravity (the "top" face).
      fill(90, 175, 70);
